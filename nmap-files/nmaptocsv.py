...
option_5 = { 'name' : ('-d', '--delimiter'), 'help' : 'Delimiter to use in CSV (semicolon if not specified)', 'nargs' : 1 }
...
def generate_csv(fd, results, output_format, header, newline, user_delimiter) :
	"""
...
def main(options, arguments):
	...
	if (options.delimiter != None) :
		generate_csv(fd_output, results, output_format, header, newline, options.delimiter)
	else :
