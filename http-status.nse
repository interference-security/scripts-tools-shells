...
description = [[
Performs a HEAD request for the root folder ("/") of a web server and displays the HTTP headers returned and saves the IP, Port and HTTP status code in RAWR CSV format. Modified from http-headers.nse script.
]]
...
action = function(host, port)
  ...
  local file = io.open("http-status-code.csv", "a")
  file:write(host.ip..","..port.number..","..result.status..",http,http\n")
  file:close()
  local file = io.open("https-status-code.csv", "a")
  file:write(host.ip..","..port.number..","..result.status..",http,ssl\n")
  file:close()
